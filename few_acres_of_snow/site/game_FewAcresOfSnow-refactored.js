...
function doLine3161(oneWeirdChar, weirdString, f, l, p, ifFranceIs1) {
    oneWeirdChar = weirdString.substr(f, 1), IsNeutralCard(oneWeirdChar) ? (l = oneWeirdChar, p = "Neutral") :
        (l = Decode(weirdString, f), p = ifFranceIs1 === 0 ? "British" : "French"), $("#ciCard" + f).mouseover(function (n, t) {
        return function () {
...
function CreateCardInfo(n, t, i, r) {
    ...
    switch (n) {
        case 0:
            weirdString = players[h].reserve;
            c = "Reserve";
            break;
        case 1:
            weirdString = players[h].deck;
            c = "Deck (" + StrDrawingPile + ")";
            break;
        case 2:
            weirdString = players[h].discardPile;
            c = StrDiscardPile;
            break;
        case 3:
            weirdString = players[h].locPile;
            c = StrAvLocCards;
            break;
        case 4:
            weirdString = players[h].empPile;
            c = StrAvEmpCards;
            break;
        case 5:
            weirdString = players[h].hand;
            c = StrHandCards;
            break;
        case 6:
            h = r === players[h].campaign ? UserIndex : (UserIndex + 1) % 2;
            weirdString = players[h].siegeCards[i];
            v = "";
            Lang === 0 ? (v = r === 0 ? "(Brit. Spieler)" : "(Franz&ouml;s. Spieler)", c = "Belagerungskarten " + v) : (v = r === 0 ? "(British player)" : "(French player)", c = "Siege cards " + v)
    }
    t && (c += " (" + StrOpponent + ")");
    var u = "",
        ifFranceIs1 = players[h].campaign,
        a = ifFranceIs1 === 0 ? "#EF9D77" : "#9DD7F2",
        b = ifFranceIs1 === 0 ? locDataEN : locDataFR,
        y = ifFranceIs1 === 0 ? empDataEN : empDataFR,
        oneWeirdChar, cardSequence, f;
    if (u += "<h2 align='center'>" + c + "<\/h2>",
        u += "<div style='position:absolute;top:70px;left:10px;width:240px;height:320px;border:1px solid black;" +
            "overflow:auto;background-color:silver'>", u += "<table border='1' cellpadding='4' style='width:100%'>", weirdString.length === 0
        ...
        for (w = 1, u += "<tr style='background-color:#D9C58A'>", u += "<td colspan='3' style='text-align:center'><b>"
            + StrTop + "<\/b>", u += "<\/td>", u += "<\/tr>", f = weirdString.length - 1; f >= 0; f--) oneWeirdChar = weirdString.substr(f, 1),
            a = IsNeutralCard(oneWeirdChar) ? "#9DCC4D" : ifFranceIs1 === 0 ? "#EF9D77" : "#9DD7F2", u += "<tr style='background-color:" + a + "'>", u += "<td style='background-color:silver;text-align:right'><b>" + w + ".<\/b><\/td>", u += "<td id='ciCard" + f + "'>", IsLocationCard(ifFranceIs1, oneWeirdChar) ? (cardSequence = Decode(weirdString, f), u += locationData[b[cardSequence][0][2]][0]) : u += IsNeutralCard(oneWeirdChar) ? empTitles[empDataN[GetNeutralIndex(oneWeirdChar)][2]] : empTitles[y[GetEmpireIndex(ifFranceIs1, oneWeirdChar)][2]], u += "<\/td>", n === 4 && (u += "<td title='costs for drafting this card' style='cursor:default'>" + y[GetEmpireIndex(ifFranceIs1, oneWeirdChar)][3], u += "<\/td>"), u += "<\/tr>", w++;
